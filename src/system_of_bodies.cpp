...
using namespace boost::numeric::odeint;
	...
	void SystemOfBodies::to_arma_vec(const std::vector<double>& y,forward_parameters &p) const{
		...
		for (int i = 0; i < n; ++i) {
			p.theta[i]  = arma::conv_to<arma::vec>::from(
			   std::vector<double>(position_vector.begin() + start_idx,
								 position_vector.begin() + start_idx + system_dofs_distribution[i])
		   );
			p.theta_dot[i] = arma::conv_to<arma::vec>::from(
				std::vector<double>(velocity_vector.begin() + start_idx,
	...
	// conversion of the arma::vec representation of the state vector back to std::vector
	...
	void SystemOfBodies::create_body(std::unique_ptr<Body> any_body) {
		...
		const auto it = bodies.insert(bodies.begin(), std::move(any_body)); // Move ownership
		...
		if (inserted_body->out_hinge_tree.size() > 0) {
