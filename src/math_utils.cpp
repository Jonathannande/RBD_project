...
arma::mat::fixed<3,3> tilde(const arma::mat& vector) {
	...
	arma::mat::fixed<3,3> tilde_matrix = {
		{0,-vector(2),vector(1)},
		{vector(2),0,-vector(0)},
		{-vector(1),vector(0),0}
	};
...
arma::mat::fixed<6,6> tilde_velocity(const arma::mat& velocity_vector) {
	...
	const arma::mat upper = join_horiz(angular_tilde,arma::zeros<arma::mat>(3, 3));
...
arma::mat::fixed<6,6> bar_velocity(const arma::mat& velocity_vector) {
	...
	const arma::mat lower = join_horiz(arma::zeros<arma::mat>(3, 3),angular_tilde);
...
arma::vec::fixed<6> coriolis_vector(const arma::mat& hinge_map,const arma::mat& velocity_vector,const arma::vec& generalized_velocities) {
	...
	return tilde_velocity_result*delta_velocity-delta_velocity_bar*delta_velocity;
}